...
- **데이터 밀집 앱에도 최적화.** 브리프가 어드민 패널·대시보드·콘솔·내부 도구이면 `reference/dashboard.md`가 기본값을 밀도 우선으로 뒤집습니다(마케팅 히어로 없음, 앱 셸 + 커맨드 팔레트, 델타 포함 KPI 카드, 헤드리스 데이터 테이블, tabular numerals, 색맹 안전 상태색, 다크 우선 엘리베이션). Linear·Stripe·Vercel·Datadog과 shadcn·Carbon 생태계의 실제 설계 방식에 근거합니다.
